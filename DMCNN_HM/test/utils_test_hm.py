        ...
        if root_dir is not None:
            for r, d, filenames in os.walk(root_cdir):
                for f in filenames:
                    ...
                    tmp_image_path = os.path.join(root_dir, f.split('-')[0]+'.png')
                    tmp_image_path = tmp_image_path.replace("..", ".")

                    self.image_clist.append(os.path.join(root_cdir, f))
                    self.image_list.append(tmp_image_path)
...
#                                                                                                                                                                          
def testAndSaveCrop(net, t_data, t_set, fout=None, out_dir='./tmp/', msg_header='', pad_size=32, crop_size=1024, idt = 1):                                                                                                  
    output_dir = out_dir
    ...
    for idx, vd in enumerate(t_data, 0):
        ...
        for x in range(pad_size, hei+pad_size, crop_size):                                                                                                                
          ...
          for y in range(pad_size, wid+pad_size, crop_size):                                                                                                              
            ...
            val_result_tmp = net((0, 0, tmp_input_pad.cuda()))
            val_result[:,:,x-pad_size:x+crop_size-pad_size,y-pad_size:y+crop_size-pad_size] = val_result_tmp[3][:,:,pad_size:pad_size+crop_size,pad_size:pad_size+crop_size] 
        ...
        for y in range(pad_size, wid+pad_size, crop_size):                                                                                                                
           ...
           val_result_tmp = net((0, 0, tmp_input_pad.cuda()))
           val_result[:,:,hei-crop_size:hei,y-pad_size:y+crop_size-pad_size] = val_result_tmp[3][:,:,pad_size:pad_size+crop_size,pad_size:pad_size+crop_size]
                                                                                                                                                                          
        tmp_input_pad = val_input_pad[:,:,hei_ex-crop_size:hei_ex+pad_size,wid_ex-crop_size:wid_ex+1*pad_size]
        val_result_tmp = net((0, 0, tmp_input_pad.cuda()))
        val_result[:,:,hei-crop_size:hei,wid-crop_size:wid] = val_result_tmp[3][:,:,pad_size:pad_size+crop_size,pad_size:pad_size+crop_size] 
        ...
        im.fromarray((o * 255.0).astype(np.uint8)).save(os.path.join(out_dir, 'original', result_path))
        im.fromarray((c * 255.0).astype(np.uint8)).save(os.path.join(out_dir, 'compressed', result_path))
        im.fromarray((p * 255.0).astype(np.uint8)).save(os.path.join(out_dir, 'predicted', result_path))
...
def evalPsnr(net2, v_data, v_set, fout=None, out_dir='./tmp/', msg_header=''):
    ...
    savePathConf(out_dir)
    cnt = 0
    for _, vd in enumerate(v_data, 0):
        com = vd['com'].float().cuda()
        org = vd['org'].float().cuda()

        org_path = vd['org_path']
        com_path = vd['com_path']
        ...
        im.fromarray((o * 255.0).astype(np.uint8)).save(os.path.join(out_dir, 'original', org_path[0].split('/')[-1]))
        im.fromarray((c * 255.0).astype(np.uint8)).save(os.path.join(out_dir, 'compressed', org_path[0].split('/')[-1]))
        im.fromarray((p * 255.0).astype(np.uint8)).save(os.path.join(out_dir, 'predicted', org_path[0].split('/')[-1]))
        cnt = cnt + 1
