...
def testNet(net, val_loader, val_set, out_dir, crop):
    ...
    with torch.no_grad():
         if crop!=True:
             ...
             p_psnr = utils_test.testAndSaveCrop(net, val_loader, val_set, fout=fout, out_dir=out_dir, pad_size=16, crop_size=512)
...
parser.add_argument('--crop', '-cr', type=bool, default=True)
...
if __name__ == "__main__":
    ...
    #jnet = model_mw.MW(3, 64)
    jnet = model_dncnn.DnCNN()
    #jnet.apply(model_mw.weights_init_kaiming)
    ...
    testNet(jnet, val_loader, val_set, args.outputdir, args.crop)
