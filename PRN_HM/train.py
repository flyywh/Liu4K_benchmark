    ...
    for epoch in epoch_range:
        ...
        for e in range(rep):
            for i, data in enumerate(training_set, 0):
                ...
                cuave0 = data['cuave0'].float().cuda()
                cuave1 = data['cuave1'].float().cuda()
                cuave2 = data['cuave2'].float().cuda()
                cuave3 = data['cuave3'].float().cuda()
                
                opt.zero_grad()
                if com.shape[0] > batch_size_big:
                    com_big = com[:batch_size_big,:,:,:]
                    org_big = org[:batch_size_big,:,:,:]
                    cuave0_big = cuave0[:batch_size_big,:,:,:]
                    cuave1_big = cuave1[:batch_size_big,:,:,:]
                    cuave2_big = cuave2[:batch_size_big,:,:,:]
                    cuave3_big = cuave3[:batch_size_big,:,:,:]
                    # print("big_size ",com_big.shape)
                    ret_big = net(com_big,cuave0_big,cuave1_big,cuave2_big,cuave3_big)
                    ...
                    # print(epoch,"  ", i, com_small.shape)

                    cuave0_small = cuave0[batch_size_big:,:,top:top + new_h,left:left + new_w]
                    cuave1_small = cuave1[batch_size_big:,:,top:top + new_h,left:left + new_w]
                    cuave2_small = cuave2[batch_size_big:,:,top:top + new_h,left:left + new_w]
                    cuave3_small = cuave3[batch_size_big:,:,top:top + new_h,left:left + new_w]
                    # print("small_size ",com_small.shape)
                    ret_small = net(com_small,cuave0_small,cuave1_small,cuave2_small,cuave3_small)
                    ...
                    ret = net(com,cuave0,cuave1,cuave2,cuave3)
                    loss = cri(ret,org)
                    loss.backward()
                    nn.utils.clip_grad_norm_(net.parameters(), 5)
                    opt.step()
...
if __name__ == "__main__":
    ...
    # training_set_small = utils.ImageDir(args.training_dir, args.training_cdir, args.training_dir1, args.training_cdir1, args.training_dir2, args.training_cdir2, preload=False,
    #                               transform=tv.transforms.Compose([
    #                                   utils.RandomCrop(32),
    #                                   utils.GenerateMultiscale(),
    #                                   utils.ToTorchTensor()
    #                               ]))
    # training_loader_small = torch.utils.data.DataLoader(
    #     training_set_small, batch_size=args.batch_size_small, num_workers=1, shuffle=True)
    ...
    jnet = nn.DataParallel(model_prn.PRN()).cuda()
    print(args.qf)
    if args.qf > 22:
        jnet.load_state_dict(torch.load('/mnt/hdd/compression-artifacts-becnmark/models/HRD4K/PRN/QP' + str(args.qf-5) + '/ckpt/' + str(epoch_mul*args.epoch_num)))
    cri = torch.nn.MSELoss().cuda()

    opt = optim.Adam(filter(lambda p: p.requires_grad, jnet.parameters()), lr=args.learning_rate)
    sch = optim.lr_scheduler.CosineAnnealingLR(opt, 16, eta_min=args.learning_rate)  # This equals to no schedule
    ...
    # cri = model.dmcnnLoss(0, 0).cuda()
    # trainNet(jnet, opt, cri, sch, args.modeldir + './ckpt', range(4, 7), training_loader, val_loader, rep=1)
    ...
    # jnet.load_state_dict(torch.load('/mnt/hdd/compression-artifacts-becnmark/models/HRD4K/PRN/QP22/ckpt/35'))
    ...
    # jnet.load_state_dict(torch.load('/mnt/hdd/compression-artifacts-becnmark/models/HRD4K/PRN/QP22/ckpt/65_withopt'))
    # load_model(jnet, opt,'/mnt/hdd/compression-artifacts-becnmark/models/HRD4K/PRN/QP37/ckpt/65_withopt' ,65)
